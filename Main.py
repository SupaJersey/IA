...
WINDOW_WIDTH = int(monitor.width * 1)
WINDOW_HEIGHT = int(monitor.height * 0.9)
...
# ---------------------------------------------------------------------------
# Top bar: pause / record / playback / scenario / output
# ---------------------------------------------------------------------------
...
def serialize_object_line(obj):
    """
    One object -> one plain-text line. Fields are separated with '|'
    (chosen because '#RRGGBB' colours and object names won't contain it).
    Custom constant forces are packed into the last field as
    'name:x:y;name:x:y;...' since a block can have any number of them.
    """
    fields = [
        obj.name,
        _fmt(obj.mass),
        _fmt(obj.dimensions[0]),
        _fmt(obj.dimensions[1]),
        _fmt(obj.position.x),
        _fmt(obj.position.y),
        _fmt(obj.angle),
        _fmt(obj.velocity.x),
        _fmt(obj.velocity.y),
        _fmt(obj.angular_velocity),
        obj.color if obj.color else "",
        _fmt(obj.material.static_friction),
        _fmt(obj.material.kinetic_friction),
        _fmt(obj.material.restitution),
    ]
    forces_field = ";".join(
        f"{f.name}:{_fmt(f.vector.x)}:{_fmt(f.vector.y)}" for f in obj.custom_forces
    )
    fields.append(forces_field)
    return "|".join(fields)
...
def parse_object_line(line):
    fields = line.split("|")
    if len(fields) < 14:
        raise ValueError(f"Malformed scenario line (expected >=14 fields): {line!r}")

    custom_forces = []
    if len(fields) > 14 and fields[14]:
        for chunk in fields[14].split(";"):
            if not chunk:
                continue
            fname, fx, fy = chunk.split(":")
            custom_forces.append((fname, float(fx), float(fy)))
    ...
    return {
        ...
        "vel_x": float(fields[7]),
        "vel_y": float(fields[8]),
        "ang_vel": float(fields[9]),
        "color": fields[10] if fields[10] else None,
        "static_friction": float(fields[11]),
        "kinetic_friction": float(fields[12]),
        "restitution": float(fields[13]),
        "custom_forces": custom_forces,
    }


def save_scenario(path, objects):
    """Plain-text scenario save: every object in the scene, one per line."""
    with open(path, "w") as f:
        f.write(f"# scenario saved {time.strftime('%Y-%m-%d %H:%M:%S')}\n")
        f.write(f"# objects: {len(objects)}\n")
        for obj in objects:
            f.write(serialize_object_line(obj) + "\n")
    ...
    with open(path, "r") as f:
        for line in f:
            ...
            if not line or line.startswith("#"):
                continue
            objects_data.append(parse_object_line(line))
    return objects_data
...
def toggle_record():
    ...
    if is_recording:
        ...
        set_output(f"Recording -> {filename}")
    else:
        if recording_file is not None:
            recording_file.close()
            recording_file = None
        record_btn.config(text="Record")
        set_output("Recording saved")
...
def toggle_playback():
    ...
    try:
        ...
        set_output("Could not read that file")
        return

    if not frames:
        set_output("No frames found in file")
        return
    ...
    set_output(f"Playing {os.path.basename(path)} ({len(frames)} frames)")
...
# Scenario buttons are created later, after their dialog functions.
# The output field reports recording, playback and scenario operations.
output_var = tk.StringVar(value="Ready")

tk.Label(top_bar, text="Output:", bg="#d9d9d9").place(x=665, y=20)

playback_status_label = tk.Entry(
    top_bar,
...
playback_status_label.place(
    x=715, y=11,
    width=max(120, WINDOW_WIDTH - 1030),
    height=34,
)
...
def open_add_block_dialog():
    ...
    # The preview polygon is drawn in screen-space pixel coordinates, so it
    # needs to be re-projected from world-space any time the camera changes
    # (zoom or pan), not just when the entry fields change. There's no
    # camera-changed event, so just keep re-drawing it on a short timer for
    # as long as this dialog is open - that keeps it glued to its world
    # position and scaling correctly as you zoom.
    ...
    dlg.protocol("WM_DELETE_WINDOW", lambda: (dialog_open.update(value=False), cleanup_preview(), dlg.destroy()))
...
grid_spacing_var = tk.DoubleVar(value=0.5)

tk.Label(viz_tab, text="Background grid", bg="white",
         font=("Arial", 10, "bold")).pack(anchor="w", padx=8, pady=(14, 2))

tk.Checkbutton(
    viz_tab, text="Show grid", variable=show_grid_var, bg="white"
).pack(anchor="w", padx=8, pady=2)

tk.Label(viz_tab, text="Grid spacing (m)", bg="white").pack(
    anchor="w", padx=8, pady=(4, 0)
)
grid_spacing_slider = tk.Scale(
    viz_tab, from_=0.1, to=5.0, resolution=0.1,
    orient="horizontal", variable=grid_spacing_var,
    bg="white", highlightthickness=0,
)
grid_spacing_slider.pack(fill="x", padx=8)

tk.Checkbutton(viz_tab, text="Force arrows", variable=show_forces_var, bg="white").pack(anchor="w", padx=8, pady=(10, 4))
...
# ---------------------------------------------------------------------------
# Scenario save / load (all objects + their properties, not the recording system)
# ---------------------------------------------------------------------------


def apply_scenario(objects_data):
    global ground_block, selected_block
    ...
    select_block(None)
    for obj in list(sim_objects):
        if obj in canvas_ids:
            main_canvas.delete(canvas_ids[obj])
            del canvas_ids[obj]
    sim_objects.clear()
    ...
    for i, data in enumerate(objects_data):
        material = Material(
            static_friction=data["static_friction"],
            kinetic_friction=data["kinetic_friction"],
            restitution=data["restitution"],
        )
        obj = Block(
            mass=data["mass"],
            dimensions=(data["width"], data["height"]),
            position=Vector2d(data["pos_x"], data["pos_y"]),
            velocity=Vector2d(data["vel_x"], data["vel_y"]),
            forces=Vector2d(0, 0),
            angle=data["angle"],
            angular_velocity=data["ang_vel"],
            material=material,
            name=data["name"],
            color=data["color"],
        )
        for fname, fx, fy in data["custom_forces"]:
            obj.custom_forces.append(ConstantForce(Vector2d(fx, fy), name=fname))

        sim_objects.append(obj)
        fill_color = obj.color if obj.color else "#333333"
        poly_id = main_canvas.create_polygon([0, 0, 0, 0, 0, 0, 0, 0],
    ...
    if new_ground is None:
        zero_mass_objects = [o for o in sim_objects if o.mass == 0.0]
        new_ground = zero_mass_objects[0] if zero_mass_objects else (sim_objects[0] if sim_objects else None)
    ground_block = new_ground

    refresh_block_list()
    render_objects()


def save_scenario_dialog():
    ...
    if not path:
        return
    try:
        save_scenario(path, sim_objects)
    except OSError:
        set_output("Could not save that scenario file")
        return
    set_output(f"Scenario saved -> {os.path.basename(path)}")
...
def load_scenario_dialog():
    ...
    try:
        objects_data = load_scenario(path)
    except (OSError, ValueError):
        set_output("Could not read that scenario file")
        return

    if not objects_data:
        set_output("No objects found in file")
        return

    apply_scenario(objects_data)
    set_output(f"Scenario loaded <- {os.path.basename(path)}")


save_scenario_btn = tk.Button(top_bar, text="Save scenario", command=save_scenario_dialog)
save_scenario_btn.place(x=310, y=13, width=110, height=30)

load_scenario_btn = tk.Button(top_bar, text="Load scenario", command=load_scenario_dialog)
load_scenario_btn.place(x=425, y=13, width=110, height=30)

# ---------------------------------------------------------------------------
# Rendering helpers
# ---------------------------------------------------------------------------

grid_render_state = {"signature": None}


def draw_background_grid(force=False):
    """Draw a world-space grid that follows the camera while staying behind objects."""
    if not show_grid_var.get():
        if grid_render_state["signature"] is not None:
            main_canvas.delete("background_grid")
            grid_render_state["signature"] = None
        return

    spacing = max(0.1, float(grid_spacing_var.get()))
    signature = (
        round(camera["scale"], 6),
        round(camera["offset_x"], 3),
        round(camera["offset_y"], 3),
        round(spacing, 3),
        canvas_width,
        canvas_height,
    )

    if not force and signature == grid_render_state["signature"]:
        return

    main_canvas.delete("background_grid")

    # Work out the visible world-space bounds.
    world_left = screen_to_world(0, 0).x
    world_right = screen_to_world(canvas_width, 0).x
    world_top = screen_to_world(0, 0).y
    world_bottom = screen_to_world(0, canvas_height).y

    # Keep the grid from becoming thousands of canvas items when zoomed far out.
    effective_spacing = spacing
    max_lines = 180
    visible_width = abs(world_right - world_left)
    visible_height = abs(world_top - world_bottom)
    required_lines = max(
        visible_width / effective_spacing,
        visible_height / effective_spacing,
    )
    if required_lines > max_lines:
        multiplier = math.ceil(required_lines / max_lines)
        effective_spacing *= multiplier

    x_start = math.floor(world_left / effective_spacing) * effective_spacing
    x_end = math.ceil(world_right / effective_spacing) * effective_spacing
    y_start = math.floor(world_bottom / effective_spacing) * effective_spacing
    y_end = math.ceil(world_top / effective_spacing) * effective_spacing

    grid_tag = "background_grid"

    x = x_start
    while x <= x_end + effective_spacing * 0.5:
        sx1, sy1 = to_screen(Vector2d(x, world_bottom))
        sx2, sy2 = to_screen(Vector2d(x, world_top))
        is_axis = abs(x) < effective_spacing * 0.5
        main_canvas.create_line(
            sx1, sy1, sx2, sy2,
            fill="#b8b8b8" if not is_axis else "#808080",
            width=1 if not is_axis else 2,
            tags=grid_tag,
        )
        x += effective_spacing

    y = y_start
    while y <= y_end + effective_spacing * 0.5:
        sx1, sy1 = to_screen(Vector2d(world_left, y))
        sx2, sy2 = to_screen(Vector2d(world_right, y))
        is_axis = abs(y) < effective_spacing * 0.5
        main_canvas.create_line(
            sx1, sy1, sx2, sy2,
            fill="#b8b8b8" if not is_axis else "#808080",
            width=1 if not is_axis else 2,
    ...
    # Always keep the grid behind blocks, arrows and debug overlays.
    main_canvas.tag_lower(grid_tag)
    grid_render_state["signature"] = signature
